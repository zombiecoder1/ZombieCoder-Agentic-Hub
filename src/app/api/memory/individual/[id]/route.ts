...
export async function PUT(
  ...
  try {
    ...
    const updates: Partial<MemoryEntry> & { memoryType?: string } = {};

    if (body.content !== undefined) updates.content = body.content;
    if (body.memoryType !== undefined) updates.memoryType = body.memoryType;
    ...
    const memory = await memoryService.updateIndividualMemory(id, updates);
...
export async function DELETE(
  ...
  try {
    const { id } = await params;
    const deleted = await memoryService.deleteIndividualMemory(id);
